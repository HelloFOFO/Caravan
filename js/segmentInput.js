...
(function( $ ) {
    ...
    var newItem = function(){
        var currentInput = $.trim($('#input_Code').val()).toString()
        if(!currentInput) return
        var item = itemTemplate.replace('__input__', currentInput)
        //插入新的Item 并将当前输入框清空


        $(item).insertBefore($("#input_Code"))
    ...
    $.fn.segInput = function( options ) {
        ...
        $('<input id="input_Code"/>').appendTo($this).css({'background-color':'#ffffff','border':'0px','margin':'2'}).attr('placeholder',settings.placeholder);
        ...
        $("#input_Code").bind("blur",function(){ newItem() });
